...
describe SQLObject do
  ...
  describe '::columns' do
    it '::columns gets the columns from the table and symbolizes them' do
      expect(Cat.columns).to eq([:id, :name, :guardian_id])
    end
    ...
    it '::columns creates getter methods for each column' do
      ...
      expect(c.respond_to? :guardian_id).to be true
    end
  ...
  describe '#initialize' do
    it '#initialize properly sets values' do
      c = Cat.new(name: 'Don Frye', id: 100, guardian_id: 4)
      expect(c.name).to eq 'Don Frye'
      expect(c.id).to eq 100
      expect(c.guardian_id).to eq 4
    end
  ...
  describe '::parse_all' do
    it '::parse_all turns an array of hashes into objects' do
      hashes = [
        { name: 'cat1', guardian_id: 1 },
        { name: 'cat2', guardian_id: 2 }
      ]
      ...
      hashes.each_index do |i|
        ...
        expect(cats[i].guardian_id).to eq(hashes[i][:guardian_id])
      end
    end
  end
  ...
  describe '#insert' do
    let(:cat) { Cat.new(name: 'Gizmo', guardian_id: 1) }

    before(:each) { cat.insert }

    it '#attribute_values returns array of values' do
      cat = Cat.new(id: 123, name: 'cat1', guardian_id: 1)
    ...
    it '#insert creates record with proper values' do
      ...
      expect(cat2.guardian_id).to eq(1)
    end
  end
